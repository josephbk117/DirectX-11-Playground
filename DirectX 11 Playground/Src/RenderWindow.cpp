...
bool RenderWindow::init(WindowContainer* winContainer, HINSTANCE hInstance, const std::string & windowTitle, const std::string & windowClass, int width, int height)
{
	...
	this->windowClassWide = StringConverter::StringToWide(this->windowClass); //wide string representation of class string (used for registering class and creating window)
	...
	this->handle = CreateWindowEx(0, //Extended Windows style - we are using the default. For other options, see: https://msdn.microsoft.com/en-us/library/windows/desktop/ff700543(v=vs.85).aspx
		...
		winContainer); //Param to create window
